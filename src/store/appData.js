...
// All static data, calculation functions, and default values
...
export const ROLES = [
  {key:"sdm",  label:"Service Delivery Manager",  l2:"A",l3:"A",enh:"A",kt:"A",  l2w:0.20,l3w:0.10,enhw:0,   ktw:0.05, desc:"Overall service accountability, client relationship, escalation management"},
  {key:"lead", label:"AMS Lead / Architect",      l2:"R",l3:"R",enh:"R",kt:"R",  l2w:0,   l3w:0.15,enhw:0.10,ktw:0.20, desc:"Technical authority, solution design, Gosu governance, team lead"},
  {key:"l2eng",label:"L2 Support Engineer",       l2:"R",l3:"C",enh:"I",kt:"R",  l2w:0.55,l3w:0,   enhw:0,   ktw:0.25, desc:"Incident resolution, break-fix, configuration, monitoring & alerting"},
  {key:"l3eng",label:"L3 Gosu Dev Engineer",      l2:"C",l3:"R",enh:"R",kt:"R",  l2w:0,   l3w:0.60,enhw:0.30,ktw:0.20, desc:"Gosu code changes, problem management, enhancement delivery, code review"},
  {key:"ba",   label:"Business Analyst",          l2:"I",l3:"I",enh:"R",kt:"C",  l2w:0,   l3w:0,   enhw:0.30,ktw:0.15, desc:"Requirements, user stories, UAT coordination, backlog management"},
  {key:"qa",   label:"QA / Test Engineer",        l2:"I",l3:"C",enh:"R",kt:"C",  l2w:0,   l3w:0,   enhw:0.20,ktw:0.10, desc:"Test planning, automation, UAT execution, regression coverage"},
  {key:"intg", label:"Integration Engineer",      l2:"R",l3:"R",enh:"C",kt:"R",  l2w:0.25,l3w:0.15,enhw:0.10,ktw:0.10, desc:"API health, integration triage, OAuth, data flows, monitoring"},
];

export const RISKS = [
  {id:1,risk:"Incumbent SI non-cooperation / slow handover",likelihood:"Medium",impact:"High",mitigation:"Contractual KT obligations; weekly progress reviews with client sponsor"},
  {id:2,risk:"Gosu code undocumented — tribal knowledge only",likelihood:"High",  impact:"High",mitigation:"Code archaeology sessions; Gosu Copilot AI assists code discovery"},
  {id:3,risk:"Integration credentials / access delays",       likelihood:"Medium",impact:"Medium",mitigation:"Early access requests Month 0; parallel provisioning in Month 1"},
  {id:4,risk:"Incident volume underestimate vs. baseline",    likelihood:"Medium",impact:"Medium",mitigation:"30% calibration buffer; agree true-up mechanism contractually"},
  {id:5,risk:"Multi-location team ramp delay",               likelihood:"Medium",impact:"Medium",mitigation:"Staggered onboarding; local leads hired and trained before go-live"},
  {id:6,risk:"Key resource attrition during KT window",      likelihood:"Low",   impact:"High",mitigation:"Minimum 2× coverage per role; runbook docs prevent single-person dependency"},
  {id:7,risk:"GW Cloud platform upgrade during KT",          likelihood:"Low",   impact:"Medium",mitigation:"Freeze window agreed with client; upgrade support built into KT plan"},
];

export const AI_ACCELERATORS = [
  {id:1,name:"GW Incident Auto-Triage",     timeline:"Y1 Q1",impact:"High",          gainIdx:0, desc:"AI assigns P1–P4, routes to correct queue, suggests runbook resolution."},
  {id:2,name:"Gosu Code Copilot",           timeline:"Y1 Q2",impact:"High",          gainIdx:1, desc:"LLM trained on Gosu patterns. Real-time suggestions, anti-pattern detection."},
  {id:3,name:"Incident Predictor",          timeline:"Y1 Q3",impact:"Medium",        gainIdx:1, desc:"ML predicts spike events (renewal, month-end) and pre-scales capacity."},
  {id:4,name:"AI Release Notes Summariser", timeline:"Y1 Q4",impact:"Medium",        gainIdx:1, desc:"Processes GW releases; generates per-module change impact assessments."},
  {id:5,name:"GW Test DataHub AI",          timeline:"Y2 Q1",impact:"High",          gainIdx:2, desc:"AI test data generation with GDPR/CDA masking for all GW entities."},
  {id:6,name:"Autonomous L2 Agent",         timeline:"Y3 Q1",impact:"Transformational",gainIdx:3,desc:"Agentic AI executes pre-approved runbooks for P3/P4 incidents autonomously."},
];
...
export function getSellRate(locKey,rates,margins,fixedSell){
  const fo=fixedSell[locKey];
  return fo!=null?fo:rates[locKey]*(1+(margins[locKey]/100));
}

export function blendedSellRate(ls,rates,margins,fixedSell){
  return LOCATIONS.reduce((s,l)=>{
    return s+(ls[l.key]/100)*getSellRate(l.key,rates,margins,fixedSell);
  },0);
}
...
export function buildPlan(base,intgHrs,ktMo,calMo,totalYrs,blendCost,blendSell,cont){
  ...
  for(let yr=1;yr<=totalYrs;yr++){
    ...
    if(yr===1){
      ...
      ssH=Math.round(annualBase*(1-g));
      ssGainIdx++;
    }
    const totalH=ktH+calH+ssH;
    const rawCost=Math.round(totalH*blendCost);
    const rawSell=Math.round(totalH*blendSell);
    rows.push({
      yr,ktH,calH,ssH,totalH,
      rawCost,rawSell,
      costWC:Math.round(rawCost*(1+cont/100)),
      sellWC:Math.round(rawSell*(1+cont/100)),
      aiGainPct:Math.round(AI_GAIN_CURVE[Math.min(ssGainIdx-1,AI_GAIN_CURVE.length-1)]*100),
      noAiHrs:annualBase,
    });
  }
  return rows;
...
export function defaultOpp(id){
  return {
    ...
    // Config
    mods:[...MODULES],
    intgs:["Policy Data Service","Payment Gateway","FNOL / Third-Party Claims","Document Management (DocuSign)","External Rating Engine"],
