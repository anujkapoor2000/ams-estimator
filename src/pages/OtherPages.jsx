...
import { T, font, RACI_COLOR, RACI_BG } from "../design.js";
import { Card, Metric, SectionHead, Btn, Slider, Input, Table, Badge, BarChart, ShoreBar, DonutChart, RaciBadge, InfoRow, Divider } from "../components/ui.jsx";
import { LOCATIONS, ROLES, RISKS, AI_ACCELERATORS, AI_GAIN_CURVE, COVERAGE_OPTIONS, buildPlan, calcBase, blendedCostRate, blendedSellRate, getSellRate, splitFTE, FTE_HRS, SP_HRS, KT_OVERHEAD } from "../store/appData.js";

// ─── Analytics ────────────────────────────────────────────────────────────────
export function AnalyticsPage({opp,opps,sym}){
  if(!opp)return<NoOpp/>;
...
// ─── Cost & Pricing ───────────────────────────────────────────────────────────
export function CostPage({opp,onUpdate,sym}){
  if(!opp)return<NoOpp/>;
...
// ─── AI & Enablers ────────────────────────────────────────────────────────────
export function AIPage({opp,sym}){
  if(!opp)return<NoOpp/>;
...
// ─── KT & Steady State ────────────────────────────────────────────────────────
export function KTPage({opp,onUpdate,sym}){
  if(!opp)return<NoOpp/>;
