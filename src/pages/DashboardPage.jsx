...
import {LOCATIONS, buildPlan, blendedCostRate, blendedSellRate, splitFTE, calcBase, FTE_HRS} from "../store/appData.js";
...
export default function DashboardPage({opp,opps,onPage,sym,fxRate,currency,onEdit}){
  ...
  const convert=v=>currency==="GBP"?Math.round(v*(fxRate||0.79)):v;
      ...
      <div style={{background:"linear-gradient(135deg,#0D1B3E 0%,#1A3A9F 60%,#2A51C1 100%)",borderRadius:T.rLg,padding:"24px 28px",marginBottom:24,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:16}}>
        <div>
          ...
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <div style={{color:"white",fontSize:22,fontWeight:800,fontFamily:font.display}}>{opp.name}</div>
            <span style={{padding:"2px 10px",borderRadius:100,background:"rgba(255,255,255,0.12)",color:"white",fontSize:10,fontWeight:700}}>{opp.version||"Draft"}</span>
            <button onClick={onEdit} style={{padding:"4px 12px",borderRadius:6,border:"1px solid rgba(255,255,255,0.3)",background:"rgba(255,255,255,0.1)",color:"white",fontSize:11,fontWeight:600,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>✏ Edit</button>
          </div>
          <div style={{color:"#7BA7F8",fontSize:13,marginTop:3}}>{opp.client}{opp.clientCity?" · "+opp.clientCity:""} · {totalYrs}-Year Plan</div>
        </div>
        <div style={{display:"flex",gap:16,alignItems:"flex-start"}}>
          {onEdit&&<button onClick={onEdit} style={{padding:"6px 14px",borderRadius:8,border:"1px solid rgba(255,255,255,0.3)",background:"rgba(255,255,255,0.1)",color:"white",fontSize:11,fontWeight:600,cursor:"pointer"}}>✏ Edit</button>}
        </div>
